 ...
 */
let l = 0, r = 0, p = 0, d = 0, u = 0;
let am = 5;

function writerCallback(data) {   
    let value = JSON.parse(data)["com"];
    if(value != undefined) {
        switch(value[0]) {
            case "left":
                r = 0; p = 0; d = 0; u = 0;                
...
// websocket server that communicates with front end.
// the client does not tell anything to the server (for now)
wss.on('connection', async (ws) => {
    ...
    // main loop that gets and sends data back.

    // while true because p, l and r will be modified whenever
    // asynchrounously. 
});
...
class Cortex {
    ...
    subRequest(stream, authToken, sessionId){
        ...
        /* Ici en fait on récupère la réponse du headset en envoyant la requête. */
        socket.on('message', (data) => {
            try {
                ...
                //throw Error("JSON parsing error in subRequest");
            }
        })
    }
    ...
    live(profileName) {
        this.socket.on('open', async ()=>{
            ...
            this.socket.on('message', writerCallback);
        })
    }
}
...
let user = {  
    "clientId":"goNpzYroTUnAfXUhRUtG9sMpSmALmyPvPe81E16l",
    "clientSecret":"9oKM42DFbnzB4JehEyvjLaGD6xJSzV6apc6GaxlRrmw6HFJH8mK2irgeZdO9iIqBAJj7MAnPMmnVuABS7dvthbSDYtbI83ntbsc7TVxyu9zYgHEOu4RqYnVnGvcp9zt4",
    "debit":5000
};
